...
<form method="POST" action="{{ route('admin.advance-salaries.update', $salary->id) }}">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col-md-6">
            ...
            <select name="driver_id" id="driver_id" class="form-select select2" required>
                ...
                <option value="{{ $driver->id }}" {{ $salary->driver_id == $driver->id ? "selected" : ""}}>{{ $driver->name }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="row mt-2">
        <div class="col-md-4">
           ...
           <input type="month"
                    ...
                    class="form-control"
                    required
                    value="{{ sprintf('%04d-%02d', $salary->year, $salary->month) }}">
        </div>
        <div class="col-md-4">
            <label>Advance Amount</label>
            <input type="number" min="1" id="amount" name="amount" class="form-control" required value="{{ $salary->amount }}">
        </div>
        <div class="col-md-4">
